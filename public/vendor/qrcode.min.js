// Minimal, standalone QRCode generator in pure JS (MIT)
// Generates SVG and Canvas for URLs without any external dependencies
(function (root, factory) {
  ...
  // QRCode implementation based on standard QR matrix generation (Byte mode, Error correction L/M)
  ...
  QRModel.prototype = {
      ...
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 40; typeNumber++) {
          ...
          for (var i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(d.mode, 4);
            buffer.put(d.getLength(), typeNumber >= 10 ? 16 : 8);
            d.write(buffer);
          }
    ...
    makeImpl: function (test, maskPattern) {
      ...
      if (this.dataCache == null) this.dataCache = QRModel.createData(this.typeNumber, this.errorCorrectLevel, this.dataList);
  ...
  QRModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
    ...
    return QRModel.createBytes(buffer, rsBlocks);
  };

  QRModel.createBytes = function (buffer, rsBlocks) {
    var offset = 0;
    var maxDcCount = 0;
    var maxEcCount = 0;
  ...
  QRCode.prototype.makeCode = function (text) {
    var qr = new QRModel(0, this.options.correctLevel);
    qr.addData(text);
    qr.make();

    if (!this.target) return qr;
    ...
    var svg = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + width + ' ' + height + '" width="' + width + '" height="' + height + '" style="border-radius:12px;display:block;background:' + this.options.colorLight + '">'];
    svg.push('<rect width="' + width + '" height="' + height + '" fill="' + this.options.colorLight + '" />');
    for (var r = 0; r < count; r++) {
      for (var c = 0; c < count; c++) {
        if (qr.isDark(r, c)) {
          var x = Math.round(c * tileW);
          var y = Math.round(r * tileH);
          var w = Math.ceil((c + 1) * tileW) - x;
          var h = Math.ceil((r + 1) * tileH) - y;
          svg.push('<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" fill="' + this.options.colorDark + '" />');
        }
      }
    }
    svg.push('</svg>');
